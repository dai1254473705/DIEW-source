...
class Curl {
    ...
    /**
     * 获得upload 的返回结果
     * @param array $files 上传的文件时的文件信息，可直接使用$_FILES数组中的数据
     * @param type $formFileName 上传时定义的form表单名称
     * @return string
     */
    public function upload($files, $formFileName = "file") {
        $re = $this->init();

        if (class_exists('\CURLFile')) {
            $fields = array($formFileName => new \CURLFile($files['tmp_name']));
        } else {
            $fields = array($formFileName => '@' . $files['tmp_name'] . ';filename=' . $files['name'] . ';type=' . $files['type']);
        }

        /**
         * 针对新版本和老版本的Curl，此处的上传直接使用curl_setopt 设置内容
         * 具体设置：参考 http://segmentfault.com/a/1190000000725185 
         */
        if ($re) {
            $this->setOptions(array("type" => "post", "return" => 1));
